...
**Validation cohort (N = 312).** Manually abstracted by the primary adjudicator, using numeric codes 0 = not police transport, 1 = police transport, 2 = police transport and EMS (mapped to the classifier categories via `GOLD_LABEL_MAP` in `classifier_lib.py`). A 62-encounter subset (an initial 32 plus an additional 30) was independently re-reviewed by the secondary adjudicator to assess inter-rater agreement on the human gold standard. **IRR result (definitive; recomputed from both adjudicators' source files, all intervals regenerated from a single seeded script): three-category Cohen's kappa = 0.761 (95% CI 0.607–0.894), percent agreement 85.5% (53/62), measured against the primary adjudicator's FINAL labels. Because those final labels incorporate consensus resolution (the secondary adjudicator's reading was adopted for two charts), that comparison is not fully independent; against the primary adjudicator's PRE-CONSENSUS codes the estimate is kappa = 0.777 (95% CI 0.614–0.915), 87.1% (54/62). Collapsed to any-police-vs-none the two are identical: kappa = 0.943 (95% CI 0.796–1.000), 61/62 (98.4%). Both comparisons are reported in the manuscript supplement; no further double-coding planned.** The primary adjudicator's master file was finalized after a small recode following the FDNY clarification. The 312 is where classifier quality is measured.
...
- **Secondary:** inter-rater agreement between the primary and secondary adjudicators on the 62-chart overlap subset, reported as Cohen's kappa. **Result: three-category kappa = 0.761 (85.5%, 53/62) against the primary adjudicator's final labels, or kappa = 0.777 (87.1%, 54/62) against their pre-consensus codes; binary any-police-vs-none kappa = 0.943 (61/62) either way** — the human benchmark against which classifier performance is read.
